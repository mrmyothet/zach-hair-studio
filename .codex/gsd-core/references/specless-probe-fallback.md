...
_gsd_lib() { for _d in "$_GSD_RT/gsd-core/bin/lib" "$_GSD_RT/bin/lib" "$_GSD_RT/.claude/bin/lib" "/workspaces/zach-hair-studio/.codex/gsd-core/bin/lib" "/workspaces/zach-hair-studio/.codex/bin/lib"; do [ -f "$_d/$1" ] && { echo "$_d/$1"; return; }; done; }
...
# consuming project's git root — falling back to git toplevel / /workspaces/zach-hair-studio/.codex (spec-phase.md:198 idiom).
_GSD_RT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
EDGE_PROBE_JS=$(for _c in \
  "$_GSD_RT/gsd-core/bin/lib/edge-probe.cjs" "$_GSD_RT/bin/lib/edge-probe.cjs" \
  "$_GSD_RT/.claude/bin/lib/edge-probe.cjs" "/workspaces/zach-hair-studio/.codex/gsd-core/bin/lib/edge-probe.cjs" \
  "/workspaces/zach-hair-studio/.codex/bin/lib/edge-probe.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)
...
`/workspaces/zach-hair-studio/.codex/gsd-core/references/prohibition-probe.md` (do not inline it). Summary:
