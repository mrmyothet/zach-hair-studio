...
- **Global scope**: `/workspaces/zach-hair-studio/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/zach-hair-studio/.codex/notes/`)
...
2. Glob `/workspaces/zach-hair-studio/.codex/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/zach-hair-studio/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/zach-hair-studio/.codex/notes/` — works in any directory
