...
for cand in \
  ...
  "/workspaces/zach-hair-studio/.codex/gsd-core/bin/gsd-tools.cjs"; do
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`/workspaces/zach-hair-studio/.codex/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just the agent. If `GSD_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.
...
global: `/workspaces/zach-hair-studio/.codex/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
